...
class P1DiningPhilosophersHomework {
    ...
    private static final class Table {
        ...
        void eat(int philosopherId) {
            ...
            // Определяем порядок захвата вилок по их id (всегда сначала с меньшим id, чтобы избежать deadlock)
            Fork first = left.id < right.id ? left : right;
            Fork second = left.id < right.id ? right : left;

            // Захватываем первую вилку
            first.lock.lock();
            try {
                // Захватываем вторую вилку
                second.lock.lock();
                try {
                    // Едим, используя обе вилки
